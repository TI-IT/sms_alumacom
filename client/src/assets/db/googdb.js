function doPost(request) {
  ...
  if (!category_id) {
    return ContentService.createTextOutput(JSON.stringify({success: false, error: `Не указан category_id`})).setMimeType(ContentService.MimeType.JSON)
  } else if (!suppliers_id) {
    return ContentService.createTextOutput(JSON.stringify({success: false, error: `Не указан поставщик suppliers_id`})).setMimeType(ContentService.MimeType.JSON)
  } else if (!product_name) {
    return ContentService.createTextOutput(JSON.stringify({success: false, error: `Не указан название товара product_name`})).setMimeType(ContentService.MimeType.JSON)
  } else if (!unit) {
    return ContentService.createTextOutput(JSON.stringify({success: false, error: `Не указан ед. измерения unit`})).setMimeType(ContentService.MimeType.JSON)
  } else if (!price) {
    return ContentService.createTextOutput(JSON.stringify({success: false, error: `Не указана цена price`})).setMimeType(ContentService.MimeType.JSON)
  }
